...
def run_robot(robot):
    ...
    epuck_node= robot.getFromDef('e_puck_sec2')
    ...
    while robot.step(timestep) != -1:
        ...
        left_speed= req_speed
        right_speed= req_speed
        ...
        if len(history_pos)>0:
            ...
            # print("line specifications for sec robot 2- ", line_sec1)
            
            circle_sec1= sensing_radius(pos)
            # print("circle specifications for sec robot 2- ", circle_sec1)
            
            if(front_wall):
                print("turn left to avoid front wall")
                # epuck_rot_field.setSFRotation([rot[0], rot[1], rot[2], -1.5716])
                left_speed = -0.5*req_speed
                right_speed = 0.5*req_speed
            else: 
                left_speed = req_speed
                right_speed = req_speed         
        ...
        # print("current position for sec robot 2- ", pos)
        #print("rotation for sec robot 1- ", rot)
        # print("previous position for sec robot 2- ", history_pos)
        history_pos= pos
        # print("-----------------------------------------------------------------------------------------------------------------------------------------------")
        # i+=1
...
if __name__ == "__main__":
    robot_sec2= Supervisor() 
    run_robot(robot_sec2)